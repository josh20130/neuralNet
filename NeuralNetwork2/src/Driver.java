...
public class Driver extends Application {
	...
	public static void main(String[] args){
		...
		while(errorFlag){
			...
			{
				double weightedSum = percep.calculateWeightedSum(data[x][0], weights);
				int result = percep.applyActivationFunction(weightedSum);
				error = data[x][1][0] - result;
				if(error != 0) errorFlag = true;
				adjustedWeights = percep.adjustWeights(data[x][0], weights, error);
